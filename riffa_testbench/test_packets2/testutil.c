...
int main(int argc, char** argv) {
	...
	if (option == 0) {
		...
		printf("OPTION 2");
		if (argc < 5) {
			printf("KAT(Known Answer Test) out chnl = 129 blocks\nUsage: %s %d <fpga id> <chnl> <channel size specified in channel tester (32 bit words)> \n", argv[0], option);
			return -1;
		}
		...
		for(idx = 0; idx < 384; idx++){
			sent_values[2*idx + 256]= sent_values[2*idx];
			sent_values[2*idx + 257]= sent_values[2*idx + 1];
		}
		GET_TIME_VAL(0);
		
		sent = fpga_send(fpga, chnl, cmd_values, 8, 0, 0, 25000);
		fprintf(stdout,"\ncmd_send_status = %d -- %d ",sent, 1);
		sent = fpga_send(fpga, chnl, sent_values, ch_size-4, 0, 1, 25000);
		...
		printf("\ntime taken : %f ms, N= %d , L= %d \n",(TIME_VAL_TO_MS(1) - TIME_VAL_TO_MS(0)),numWords,numLoops);
